...
class SRCNN(object):
  ...
  def train(self, config):
    ...
    if config.is_train:
      ...
      bg=0
      en=0
      names = get_name(config.input, str(config.label_size))
      dimen = get_dimen(config)
      writedata = []
      for i in range(len(nxy)):

        if i%24 ==0:
          print('Reconstructing   ' + names[(i//24)] +':')
        ...
        result,img = self.sess.run([self.pred,self.images],feed_dict={self.images: train_data[bg:en]})
        ...
        print('{0:11}/24   |'.format(i%24+1)+'██'*(i%24+1)+'  '*(23-i%24)+'|',end='\r')
        sys.stdout.flush()
        ...
        if (i+1) % 24 == 0:
          ...
          writedata = []
          print('{0:11}/24   |'.format(i%24+1)+'██'*(i%24+1)+'  '*(23-i%24)+'|   COMPLETE!')
          sys.stdout.flush()
          '''
