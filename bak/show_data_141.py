...
def main(_):
	...
	data, label_ = preprocess(FLAGS)
	image,feature,label = data[:,:,:,0],data[:,:,:,1],label_[:,:,:,0]
	#image = get_data(FLAGS.input)
	#label = get_data(FLAGS.label)
	#feature = get_data(FLAGS.feature)
	feature = feature/20+10
	...
	for i in range(image.shape[0]):
		fig = plt.figure('learning',figsize=(20,20))
		plt.subplot(2,2,3)
		plt.title('input range['+str(np.min(image[i]))+' , '+str(np.max(image[i]))+']')
		plt.imshow(image[i],cmap=cm.s3pcpn)
		...
		#plt.imshow(feature[0],cmap=plt.cm.gist_earth)
		plt.subplot(2,2,4)
		plt.title('label range['+str(np.min(label[i]))+' , '+str(np.max(label[i]))+']')
		plt.imshow(label[i],cmap=cm.s3pcpn)
